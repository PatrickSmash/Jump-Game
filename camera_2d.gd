...
func _update_dragon_position():
	if not dragon_sprite or not chase_active:
		return
	
	# Position the dragon at the death zone location
	...
	# Make sure the dragon is visible when the chase is active
	dragon_sprite.visible = chase_active
